#!/usr/bin/env python3
"""Check connected integer execution against an independent architectural ledger."""
import argparse
...
def main():
    ...
    config = json.loads((ROOT/'config/integer_backend.json').read_text())
    if (config['queue_entries'], config['rob_entries'], config['identity_bits'], config['physical_registers'], config['producer_slots']) != (16, 32, 13, 64, 2):
        raise RuntimeError('unsupported backend geometry')
    OUT.mkdir(parents=True, exist_ok=True)
    retained = ROOT/'evidence/receipts/integer_backend.json'
    retained.parent.mkdir(parents=True, exist_ok=True)
    retained.unlink(missing_ok=True)
    receipt = OUT/'receipt.json'
    receipt.unlink(missing_ok=True)
    paths = [*config['sources'], 'verif/unit/integer_backend_tb.cpp', 'config/integer_backend.json',
             ...
             'tools/run_integer_backend.py', 'config/toolchain.lock', 'config/synthesis.lock', 'Makefile']
    ...
    def build(source_list, directory, log, mutated=False):
        run(['verilator', '--cc', '--exe', '--build', '--build-jobs', '2', '--Wall', '--assert',
             *(['-DSYNTHESIS', '-Wno-UNUSEDSIGNAL', '-Wno-UNUSEDPARAM'] if mutated else []),
             '--top-module', 'integer_backend', '--Mdir', directory,
             '-CFLAGS', f'-std=c++17 -Wall -Wextra -Werror -I{OUT}', lint_config, *source_list,
             ROOT/'verif/unit/integer_backend_tb.cpp', '-o', 'integer_backend_check'], log)
        return directory/'integer_backend_check'

    run(['verilator', '--lint-only', '--Wall', '--assert', '--top-module', 'integer_backend',
    ...
    required = {'dual_allocate', 'dual_complete', 'dual_retire', 'dual_issue', 'queue_full', 'rob_full',
                'completion_stall', 'retire_stall', 'issue_stall', 'unsupported', 'flush_held', 'reset_held',
                ...
                *(f'op_{n}' for n in range(21))}
    for seed in config['seeds']:
        output = run([binary, str(seed), str(config['random_cycles_per_seed'])], f'seed_{seed}.log')
        if not output.startswith('INTEGER BACKEND PASS ') or len(output.splitlines()) != 1:
    ...
    negatives = {'prefix': 'INTEGER_PREFIX', 'held_drain': 'INTEGER_DRAIN'}
    for name, marker in negatives.items():
        run([binary, 'negative', name], 'negative_'+name+'.log', failure=marker)
        print(f'Integer backend caller assertion {name}: PASS (rejected)', flush=True)

    mutations = {
        'lost_hold': ('alu_pipeline.sv', 'if (take_i) pending_q <= 0;', 'pending_q <= 0;'),
        'lost_refill': ('alu_pipeline.sv', '(!pending_q || take_i)', '!pending_q'),
        'lost_identity': ('alu_pipeline.sv', 'id_o <= id_i;', "id_o <= 13'd0;"),
        'wrong_result': ('alu_pipeline.sv', 'built.rd_value = result;', "built.rd_value = result ^ 32'd1;"),
        'wrong_next_pc': ('alu_pipeline.sv', 'built.pc_after = next_pc;', 'built.pc_after = pc_i;'),
        'wrong_source_event': ('alu_pipeline.sv', 'built.rs1_value = source1_i;', "built.rs1_value = source1_i ^ 32'd1;"),
        'lost_second_launch': ('integer_backend.sv', '.launch_i(issue_o[lane])', '.launch_i(issue_o[lane] && lane == 0)'),
        'swapped_operands': ('integer_backend.sv', '.source1_i(read_data[lane*64 +: 32])', '.source1_i(read_data[lane*64+32 +: 32])'),
        'swapped_pc': ('integer_backend.sv', '.pc_i(issue_payload[lane*64+32 +: 32])', '.pc_i(issue_payload[lane*64 +: 32])'),
        'ignored_completion_grant': ('integer_backend.sv', 'completion_valid_o & completion_enable_i', 'completion_valid_o'),
        'ignored_execution_stall': ('integer_backend.sv', 'producer_ready && execution_ready_i[lane]', 'producer_ready'),
        'unsupported_dispatch': ('integer_backend.sv', '.resources_ready_i((valid_i & ~supported_o) == 0)', ".resources_ready_i(1'b1)"),
        'raw_completion_release': ('integer_backend.sv', '.take_i(completion_accept_o[lane])', '.take_i(completion_valid_o[lane])'),
        'lost_producer_flush': ('integer_backend.sv', '.clk_i, .rst_i, .flush_i, .launch_i', ".clk_i, .rst_i, .flush_i(1'b0), .launch_i"),
    }
    for name, (filename, old, new) in mutations.items():
        ...
        run([executable, '1', '0'], name+'.log', failure='integer backend mismatch')
        print(f'Integer backend mutation {name}: PASS (detected)', flush=True)

    script = OUT/'synth.ys'
    script.write_text('plugin -i slang\nread_slang --top integer_backend '+' '.join(str(p) for p in sources)+'\n'
                      f'synth -top integer_backend -flatten\ncheck -assert\nwrite_json {OUT/"synth.json"}\n')
    ...
    archive = ROOT/f'evidence/integer_backend/{run_id}.tar.gz'
    ...
    print(f'Integer backend: PASS; {sum(counts.values())} generic cells; receipt {retained.relative_to(ROOT)}', flush=True)
