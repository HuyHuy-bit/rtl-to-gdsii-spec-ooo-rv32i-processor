...
module frontend_fault_decode (
  ...
  for (genvar lane = 0; lane < 2; lane++) begin : lanes
    ...
    assign fault_o[lane] = valid_i[lane] && (fetch_fault_i || decoded[lane].op == OP_ILLEGAL);
    assign cause_o[lane*32 +: 32] = fetch_fault_i ? fetch_cause_i : CAUSE_ILLEGAL_INSTRUCTION;
    assign value_o[lane*32 +: 32] = fetch_fault_i ? pc_i[lane*32 +: 32] : instruction_i[lane*32 +: 32];
  end
endmodule
