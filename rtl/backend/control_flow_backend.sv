...
module control_flow_backend (
  ...
  assign selected = {valid_i[1] && !cfi[0], valid_i[0]};
  ...
  for (genvar lane = 0; lane < 2; lane++) begin : lanes
    ...
    assign supported_o[lane] = (cfi[lane] || decoded[lane].op inside {OP_ALU, OP_LUI, OP_AUIPC})
      && pc_i[lane*32 +: 2] == 0;
  ...
  control_flow_pipeline producer0 (
    .clk_i, .rst_i, .flush_i, .launch_i(issue_o[0]), .ready_o(producer_ready[0]), .busy_o(producer_busy_o[0]),
    ...
    .valid_o(completion_valid_o[0]), .id_o(completion_id_o[12:0]), .event_o(completion_event_o[0]),
  ...
  alu_pipeline producer1 (
    .clk_i, .rst_i, .flush_i(flush_i || cancel_alu1), .launch_i(issue_o[1]), .ready_o(producer_ready[1]),
    ...
    .valid_o(completion_valid_o[1]), .id_o(completion_id_o[25:13]), .event_o(completion_event_o[1]),
  ...
  issue_backend backend (
    ...
    .valid_i, .cfi_i(cfi), .solo_i(2'b00), .rs1_i(rs1), .rs2_i(rs2), .rd_i(rd), .pc_i,
    ...
    .trap_ready_i(1'b0), .trap_valid_o(fault_pending_o), .trap_accept_o(), .trap_event_o(fault_event_o),
