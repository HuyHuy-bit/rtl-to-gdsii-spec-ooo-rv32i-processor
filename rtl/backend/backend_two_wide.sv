...
module backend_two_wide (
  ...
  for (genvar lane = 0; lane < 2; lane++) begin : completion_control
    assign write_offer[lane] = complete_offer_i[lane] && !complete_event_i[lane].trap;
  ...
  assign wb_data_o = {complete_event_i[1].rd_value, complete_event_i[0].rd_value};
  ...
  rename_recovery rename_owner (
    ...
    .wb_offer_i(write_offer), .wb_live_i(rob_complete_ready), .wb_destination_i(wb_destination_o),
  ...
  rob_two_wide rob (
    ...
    .complete_ready_o(rob_complete_ready), .complete_accept_o, .complete_destination_o(wb_destination_o),
...
`ifndef SYNTHESIS
  always_ff @(posedge clk_i) begin
    if (!rst_i) begin
      ...
      assert ((wb_accept_o & ~complete_accept_o) == 0 && checkpoint_accept_o == (|(allocate_accept_o & cfi_i)))
        else $fatal(1, "BACKEND_ATOMIC_RESULT");
      assert (!drained_i || (valid_i == 0 && complete_offer_i == 0 && !resolve_offer_i))
